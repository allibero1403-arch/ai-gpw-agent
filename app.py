...
with tab2:
    ...
    if st.button("🔎 Szukaj", type="primary"):
        ...
        st.dataframe(filt[cols_display].style.format({
            ...
            "Revenue (TTM)": lambda x: format_currency(x, st.session_state.currency),
            "Free Cash Flow (TTM)": lambda x: format_currency(x, st.session_state.currency),
            "Net Income (TTM)": lambda x: format_currency(x, st.session_state.currency),
            "Signal Score": "{:.2f}"
        }), use_container_width=True, hide_index=True)
    ...
    with col_right:
        ...
        if tickers_list:
            ...
            for idx, row in df_tickers.iterrows():
                ...
                # ✅ 1 POZYCJA = 1 LINIA - z nagłówkiem tabeli
                if idx == 0:
                    st.markdown("""
                    <div class="pt-header">
